...
$req2 = mysqli_query($con,'SELECT m1.id, m1.title, m1.timestamp, count(m2.id) AS reps, users.id AS userid, users.name FROM pm AS m1, pm AS m2,users WHERE ((m1.user1="'.$_SESSION['usr_id'].'" AND m1.user1read="yes" AND users.id=m1.user2) OR (m1.user2="'.$_SESSION['usr_id'].'" and m1.user2read="yes" and users.id=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc');
?>
<?php echo $_SESSION['usr_name']; ?>'s Messages:<br />
